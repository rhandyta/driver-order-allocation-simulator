...
def run_experiment_cr(config, output_dir):
    weights, sub_weights, temperature, norm_params = _load_from_config(config)
    os.makedirs(os.path.join(output_dir, 'csv'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'charts'), exist_ok=True)
    
    cr_values = [0.70, 0.80, 0.90, 0.95, 0.99, 1.00]
    iterations = 1000
    market = Market(area="area_A", active_drivers=len(cr_values), active_orders=100)
    
    drivers = [_make_driver(id=f"Driver_{cr:.2f}", cr=cr) for cr in cr_values]
    ...
    print("Experiment CR Results:")
    with open(os.path.join(output_dir, 'csv', 'experiment_cr.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Driver', 'CR', 'Wins', 'Win_Rate'])
        for d in drivers:
            rate = win_counts[d.id] / iterations
            print(f"{d.id}: {win_counts[d.id]} wins ({rate*100:.1f}%)")
            writer.writerow([d.id, d.completion_rate, win_counts[d.id], rate])
            
    plt.figure(figsize=(10, 6))
    plt.bar([str(cr) for cr in cr_values], [win_counts[f"Driver_{cr:.2f}"]/iterations for cr in cr_values])
    plt.title('Win Rate by Completion Rate')
    plt.xlabel('Completion Rate')
    plt.ylabel('Win Rate')
    plt.savefig(os.path.join(output_dir, 'charts', 'experiment_cr.png'))
    plt.close()


def run_experiment(config, output_dir="results"):
    return run_experiment_cr(config, output_dir)
