...
def run_experiment_history(config, output_dir):
    weights, sub_weights, temperature, norm_params = _load_from_config(config)
    os.makedirs(os.path.join(output_dir, 'csv'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'charts'), exist_ok=True)
    
    match_values = [0, 0.25, 0.5, 0.75, 1.0]
    iterations = 1000
    market = Market(area="area_A", active_drivers=len(match_values), active_orders=100)
    
    drivers = []
    for mv in match_values:
        total_trips = 100
        match_trips = int(total_trips * mv)
        hist = {
            "services": {"GoRide": match_trips, "GoFood": total_trips - match_trips, "GoSend": 0},
            "areas": {"area_A": match_trips, "area_B": total_trips - match_trips, "area_C": 0},
            "time_slots": {"morning": match_trips, "evening": total_trips - match_trips},
            "distance_buckets": {"0-3km": match_trips, "3-7km": total_trips - match_trips, "7km+": 0}
        }
        drivers.append(_make_driver(id=f"Driver_Hist_{mv:.2f}", history=hist))
        
    win_counts = {d.id: 0 for d in drivers}
    
    for i in range(iterations):
        order = generate_random_order(order_id=f"O_{i}")
        order.service_type = "GoRide"
    ...
    print("Experiment History Results:")
    with open(os.path.join(output_dir, 'csv', 'experiment_history.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Driver', 'History_Match', 'Wins', 'Win_Rate'])
        for d, mv in zip(drivers, match_values):
            rate = win_counts[d.id] / iterations
            print(f"{d.id}: {win_counts[d.id]} wins ({rate*100:.1f}%)")
            writer.writerow([d.id, mv, win_counts[d.id], rate])
            
    plt.figure(figsize=(10, 6))
    plt.bar([str(mv) for mv in match_values], [win_counts[f"Driver_Hist_{mv:.2f}"]/iterations for mv in match_values])
    plt.title('Win Rate by History Match Proportion')
    plt.xlabel('History Match Proportion')
    plt.ylabel('Win Rate')
    plt.savefig(os.path.join(output_dir, 'charts', 'experiment_history.png'))
    plt.close()


def run_experiment(config, output_dir="results"):
    return run_experiment_history(config, output_dir)
