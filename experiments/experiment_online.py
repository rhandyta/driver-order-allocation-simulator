...
def run_experiment_online(config, output_dir):
    weights, sub_weights, temperature, norm_params = _load_from_config(config)
    os.makedirs(os.path.join(output_dir, 'csv'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'charts'), exist_ok=True)
    
    online_days_list = [2, 5, 7, 10, 14]
    iterations = 1000
    market = Market(area="area_A", active_drivers=len(online_days_list), active_orders=100)
    
    drivers = [_make_driver(id=f"Driver_{od}d", online_days=od, online_hours=od*5) for od in online_days_list]
    ...
    print("Experiment Online Results:")
    with open(os.path.join(output_dir, 'csv', 'experiment_online.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Driver', 'Online_Days', 'Wins', 'Win_Rate'])
        for d in drivers:
            rate = win_counts[d.id] / iterations
            print(f"{d.id}: {win_counts[d.id]} wins ({rate*100:.1f}%)")
            writer.writerow([d.id, d.online_days, win_counts[d.id], rate])
            
    plt.figure(figsize=(10, 6))
    plt.bar([str(od) for od in online_days_list], [win_counts[f"Driver_{od}d"]/iterations for od in online_days_list])
    plt.title('Win Rate by Online Days')
    plt.xlabel('Online Days')
    plt.ylabel('Win Rate')
    plt.savefig(os.path.join(output_dir, 'charts', 'experiment_online.png'))
    plt.close()


def run_experiment(config, output_dir="results"):
    return run_experiment_online(config, output_dir)
