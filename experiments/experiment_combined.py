...
def run_experiment_combined(config, output_dir):
    weights, sub_weights, temperature, norm_params = _load_from_config(config)
    os.makedirs(os.path.join(output_dir, 'csv'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'charts'), exist_ok=True)
    
    iterations = 1000
    market = Market(area="area_A", active_drivers=3, active_orders=100)
    
    d1 = _make_driver("Elite_Driver", ar=0.99, cr=0.99, online_days=14, online_hours=120)
    d2 = _make_driver("Avg_Driver", ar=0.85, cr=0.90, online_days=7, online_hours=50)
    d3 = _make_driver("New_Driver", ar=1.0, cr=1.0, online_days=2, online_hours=10)
    ...
    print("Experiment Combined Results:")
    with open(os.path.join(output_dir, 'csv', 'experiment_combined.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Driver', 'Wins', 'Win_Rate'])
        for d in drivers:
            rate = win_counts[d.id] / iterations
            print(f"{d.id}: {win_counts[d.id]} wins ({rate*100:.1f}%)")
            writer.writerow([d.id, win_counts[d.id], rate])
            
    plt.figure(figsize=(10, 6))
    plt.pie([win_counts[d.id] for d in drivers], labels=[d.id for d in drivers], autopct='%1.1f%%')
    plt.title('Win Share among Driver Profiles')
    plt.savefig(os.path.join(output_dir, 'charts', 'experiment_combined.png'))
    plt.close()


def run_experiment(config, output_dir="results"):
    return run_experiment_combined(config, output_dir)
